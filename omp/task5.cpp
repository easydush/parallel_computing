...
int main()
{
    ...
    // fill array with numbers from 0 to 50
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < 8; j++)
        {
...
#pragma omp section
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                printf("Thread %d is computing average\n", omp_get_thread_num());
                for (int j = 0; j < 8; j++)
                {
                    sum += d[i][j];
                    length += 1;
...
#pragma omp section
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                printf("Thread %d is counting for elements mod 3 == 0\n", omp_get_thread_num());
                for (int j = 0; j < 8; j++)
                {
                    if (d[i][j] % 3 == 0)
                        count3 += 1;
            ...
            printf("Number of elements mod 3 == 0: %d\n", count3);
        }
    }
}
